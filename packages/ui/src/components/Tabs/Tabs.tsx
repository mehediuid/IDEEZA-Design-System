    ...
    return (
      <TabsContext.Provider value={{ variant, size, value: current, onValueChange: change }}>
        <div
          ...
          role="tablist"
          className={cx("ids-tabs", `ids-tabs--${variant}`, fill ? "ids-tabs--stretch" : null, className)}
          {...props}
        >
          {box && (
            <span
              ...
              className={cx(
                "ids-tabs__indicator",
                `ids-tabs__indicator--${style}`,
                style !== "line" ? (size === "sm" ? "ids-tabs__indicator--r6" : "ids-tabs__indicator--r8") : null,
                placed.current ? "ids-tabs__indicator--travel" : null
              )}
              style={
                style === "line"
  ...
  ({ className, value, counter, fill, disabled, children, onClick, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cx(
          "ids-tab",
          `ids-tab--${style}-${ctx.size}`,
          `ids-tab--type-${ctx.size === "lg" ? "md" : "sm"}`,
          `ids-tab--${style}-${disabled ? "disabled" : selected ? "active" : "idle"}`,
          fill ? "ids-tab--stretch" : null,
          className
        )}
        {...props}
      >
        {children}
        {counter !== undefined && (
          <span className="ids-tab__counter">
            {counter}
          </span>
        )}
      </button>
